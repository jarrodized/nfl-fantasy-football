...
import datetime
# game key for 2018 - 380
# game key for 2019 - 390
...
class UpdateData():
    ...
    def getGameKeys(self):
        ...
        url = 'https://http://fantasysports.yahooapis.com/fantasy/v2/game/nfl'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
        with open('./game_keys/game_keys.json', 'w') as outfile:
            json.dump(r, outfile)
            return;
    ...
    def UpdateTransactions(self):
        ...
        yahoo_api._login() # get the newest transactions and write over the existing new_transactions
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/390.l.182974/transactions'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateLeague(self):
        # LEAGUE OVERVIEW
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/390.l.182974/'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateLeagueStandings(self):
        # STANDINGS
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/390.l.182974/standings'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateScoreboards(self):
        ...
        while week < num_weeks+1: #assumes 16 week-schedule
            url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/390.l.182974/scoreboard;week='+str(week)
            response = oauth.session.get(url, params={'format': 'json'})
            r = response.json()
    ...
    def UpdateRosters(self):
        ...
        for week in range(1, num_weeks+1): #assumes 16-week schedule
            ...
            for team in range(1, num_teams+1): #assumes 12-team league
                url = 'https://fantasysports.yahooapis.com/fantasy/v2/team/390.l.182974.t.'+str(team)+'/roster;week='+str(week)
                response = oauth.session.get(url, params={'format': 'json'})
                r = response.json()
